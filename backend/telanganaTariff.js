...
function getTariffBand(monthlyUnits = 0) {
...
function getBillingCycleLabel(date = new Date()) {
...
function getBillingDueDateLabel(date = new Date()) {
...
function getMinimumCharge(phaseType = DEFAULT_PHASE_TYPE, contractedLoad = DEFAULT_CONTRACTED_LOAD) {
...
function getMinimumChargeRule(phaseType = DEFAULT_PHASE_TYPE, contractedLoad = DEFAULT_CONTRACTED_LOAD) {
  if (normalizePhaseType(phaseType) === "three") {
...
function getNextTariffThreshold(monthlyUnits = 0) {
  const units = normalizeUnits(monthlyUnits);
...
function getTariffWarningMessage(currentUnits = 0, projectedUnits = 0) {
...
function projectMonthlyUnits({ monthlyUnits, latestReading, history = [], dailyUsage = [] } = {}) {
...
function calculateTelanganaLtIDomesticBill(input = {}) {
...
function buildBillingForecastSeries({
  bill = null,
  latestReading = null,
  history = [],
  dailyUsage = [],
